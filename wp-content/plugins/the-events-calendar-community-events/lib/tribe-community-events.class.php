	...
	class TribeCommunityEvents {
		...
		public function maybeLoadAssets() {
			if ( tribe_is_community_my_events_page() || tribe_is_community_edit_event_page() ) {
				...
				wp_enqueue_style( 'chosen-style', $tec->pluginUrl . 'resources/chosen.css' );
				wp_enqueue_script( 'chosen-jquery', $tec->pluginUrl . 'resources/chosen.jquery.min.js', array('jquery'), '0.9.5', false );
				wp_enqueue_style( TribeEvents::POSTTYPE.'-admin-ui', $tec->pluginUrl . 'resources/events-admin-ui.css' );
				wp_enqueue_script( 'jquery-ui-datepicker', $tec->pluginUrl . 'resources/ui.datepicker.min.js', array('jquery-ui-core'), '1.7.3', true );
				wp_enqueue_script( 'jquery-ui-dialog', $tec->pluginUrl . 'resources/ui.dialog.min.js', array('jquery-ui-core'), '1.7.3', true );
				wp_enqueue_script( 'jquery-ecp-plugins', $tec->pluginUrl . 'resources/jquery-ecp-plugins.js', array('jquery') );
				wp_enqueue_script( TribeEvents::POSTTYPE.'-admin', $tec->pluginUrl . 'resources/events-admin.js', array('jquery-ui-datepicker'), '', true );
		...
		public function disable_comments_on_page( ){
			return $this->getTemplatePath( 'views', 'blank-comments-template.php' );
		}
		...
		public function doEventForm( $id = null ) {
			...
			if ( isset( $_GET['action'] ) && $_GET['action'] == 'deleteFeaturedImage' && wp_verify_nonce( $_GET['_wpnonce'], 'tribe_community_events_featured_image_delete' ) && $this->userCanEdit( $tribe_event_id, TribeEvents::POSTTYPE ) ) {
			...
			if ( $this->allowAnonymousSubmissions || is_user_logged_in() ) {
				...
				if ( is_user_logged_in() && $this->allowUsersToEditSubmissions )
					$output .= '<div id="my-events"><a href="' . $this->getUrl( 'list' ) . '" class="button">' . apply_filters( 'tribe_ce_event_list_button_text', __( 'My Events', 'tribe-events-community' ) ) . '</a></div>';
		...
		public function doMyEvents( $page = null ) {
			...
			if ( !$this->allowUsersToEditSubmissions ) {
				do_action( 'tribe_ce_event_list_not_enabled_page' );
				return apply_filters( 'tribe_ce_event_list_not_enabled_text', __( 'This feature is not currently enabled.', 'tribe-events-community' ) );
			}
		...
		/**
		 * Grant the edit_tribe_events capability to subscribers.
		 *
		 * @since 1.0.3
		 * @author PaulHughes01
		 *
		 * @return void
		 */
		public function addSubscriberEditCapabilities() {
